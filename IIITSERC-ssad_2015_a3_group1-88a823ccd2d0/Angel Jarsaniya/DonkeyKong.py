...
class Donkey(Person):
	...
	def move(self,b):
		...
		if self.checkPlayer(b)==True:
			...
			self.__position = self.updatedposition
			return -1			
		elif self.checkWall(b)==True or self.checkDonkey(b)==True:
			pass
...
def main():
	try:
		...
		while y!='q':
			...
			for i in range(0,numofDonkeys):
				if(g[i].move(b)==-1) or p.getPlayerPosition()==g[i].getDonkeyPosition():
					tobreakloop = True			
					break
